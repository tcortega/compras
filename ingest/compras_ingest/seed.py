...
def main() -> int:
    ...
    if seen_uf != {"RJ", "SP", "RS", "SC", "MG", "PR", "BA", "PE", "GO", "ES", "PB", "CE"}:
        raise SystemExit(f"warehouse UF set is not RJ+SP+RS+SC+MG+PR+BA+PE+GO+ES+PB+CE: {sorted(seen_uf)}")
    ...
    if counts["orgao"] < 13:
        raise SystemExit(f"warehouse orgao count {counts['orgao']} < 13")
    print("seed ok")
