...
def main() -> int:
    ...
    if seen_uf != {"RJ", "SP", "RS", "SC", "MG", "PR", "BA", "PE", "GO", "ES"}:
        raise SystemExit(f"warehouse UF set is not RJ+SP+RS+SC+MG+PR+BA+PE+GO+ES: {sorted(seen_uf)}")
    ...
    if counts["orgao"] < 11:
        raise SystemExit(f"warehouse orgao count {counts['orgao']} < 11")
    print("seed ok")
