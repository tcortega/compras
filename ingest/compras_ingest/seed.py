...
def main() -> int:
    ...
    if seen_uf != {"RJ", "SP", "RS", "SC", "MG", "PR", "BA", "PE"}:
        raise SystemExit(f"warehouse UF set is not RJ+SP+RS+SC+MG+PR+BA+PE: {sorted(seen_uf)}")
    ...
    if counts["orgao"] < 9:
        raise SystemExit(f"warehouse orgao count {counts['orgao']} < 9")
    print("seed ok")
