...
def main() -> int:
    ...
    if seen_uf != {"RJ", "SP"}:
        raise SystemExit(f"warehouse UF set is not RJ+SP: {sorted(seen_uf)}")
    landed = {(str(o.get("municipioIbge") or ""), str(o.get("uf") or "")) for o in fetch_orgaos(settings)}
    if landed != {("3306305", "RJ"), ("3303302", "RJ"), ("3506003", "SP")}:
    ...
    if counts["orgao"] < 3:
        raise SystemExit(f"warehouse orgao count {counts['orgao']} < 3")
    print("seed ok")
